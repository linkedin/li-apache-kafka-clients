...
import static com.linkedin.kafka.clients.headers.DefaultHeaderSerde.utf8StringLength;
...
public class DefaultHeaderSerializerTest {
  ...
  public void headerSerializationTest() {
    ...
    Assert.assertEquals(headerSerializer.serializedHeaderSize(headers),
    ...
    Assert.assertEquals(bbuf.remaining(), headerSerializer.serializedHeaderSize(headers));
    ...
    Assert.assertFalse(deserializeResult.value().hasRemaining());
    Assert.assertEquals(deserializeResult.headers().size(), 1);
    Assert.assertEquals(deserializeResult.headers().get(headerKey), headerValue.getBytes());
  }

  @Test
  public void utf8LengthTest() {
    Assert.assertEquals(utf8StringLength(""), 0);
    Assert.assertEquals(utf8StringLength("\0"), 1);
    Assert.assertEquals(utf8StringLength("a"), 1);
    Assert.assertEquals(utf8StringLength("\u007f"), 1);
    Assert.assertEquals(utf8StringLength("\u0080"), 2);
    Assert.assertEquals(utf8StringLength("\u07FF"), 2);
    Assert.assertEquals(utf8StringLength("\u0800"), 3);
    Assert.assertEquals(utf8StringLength("\uFFFF"), 3);
    String startSupplemental = new String(new int[] { 0x10000}, 0, 1);
    String endSupplemental = new String(new int[] { 0x10FFFF}, 0, 1);
    Assert.assertEquals(utf8StringLength(startSupplemental), 4);
    Assert.assertEquals(utf8StringLength(endSupplemental), 4);
    String doughnut = new String(new int[] { 0x1F369}, 0, 1);
    Assert.assertEquals(utf8StringLength(doughnut), 4);
  }
}
